...
// A key with a matching file links to its row; an unresolved one renders as a
// plain chip showing the key, mirroring how unresolved term cards degrade. An
// empty key yields no chip at all: a story may name no actor. prefix is the
// relative path back to the output root from the page doing the rendering.
