...
func (b *Builder) buildMappings() error {
	...
	for _, f := range files {
		...
		terms := b.resolveTermCards(em.Terms)

		outPath := filepath.Join(b.OutDir, "mapping", em.StoryKey.Value+".html")
		if err := b.buildMapping(outPath, em, storyName, storyPath, terms); err != nil {
			return err
		}
...
func (b *Builder) buildMapping(path string, em *domain.ExampleMapping, storyName, storyPath string, terms []termCard) error {
	f, err := os.Create(path)
	if err != nil {
		return err
	}
	defer f.Close()
	return renderMapping(f, em, storyName, storyPath, terms)
}
