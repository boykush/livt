...
|------|-----------|---------|
| `list_opportunities` | — | Every [opportunity](../guides/opportunities.md) with its key, name, and statement. Each entry links to its opportunity resource (`uri`), to its canvas (`canvas_uri`) when one has been filled in, and to its `story_map` when one has been drawn — an opportunity has one map, filed under its key. A missing canvas or story map is the record that the opportunity has not been taken that far. |
...
| URI | Returns |
|-----|---------|
| `livt://opportunity/{opportunity_key}` | An opportunity: its name, its statement (whose problem, and what the business gets from solving it), and its frontmatter meta. Carries `canvas_uri` when a canvas has been filled in, and `story_map` — the map filed under its key, as `list_story_maps` gives it — when one has been drawn. |
