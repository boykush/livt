...
| `list_stories` | `opportunity` (optional) — an opportunity key, as `list_opportunities` hands it out; keeps only the stories on the map with that key, and an unknown key yields an empty list | Every story with its key and name. Each entry links to its story resource (`uri`); stories that have an example mapping also include `example_mapping_uri`, and stories on a story map carry `opportunities` — one per map they sit on, with the opportunity's `key`, `name`, and `uri`. Where no opportunity file describes the map's key, the map [stands in](../guides/opportunities.md#linking-a-story-map) with its own name and story map resource URI. |
| `list_example_mappings` | `opportunity` (optional) — as for `list_stories`; a mapping whose key sits on no map, as one with no story usually does, matches none | Every example mapping, including one whose story has no card, with its story key and mapping resource URI (`uri`). A mapping carries `name` when it has one of its own and `story_uri` when a story file exists — read that story for the name of a mapping with none of its own — and `opportunities`, as `list_stories` gives them for the maps its key sits on. |
| `list_story_maps` | — | Every story map — one per opportunity — with the `opportunity_key` it is filed under, its `name`, and its story map resource URI (`uri`). |
...
| URI | Returns |
|-----|---------|
| `livt://opportunity/{opportunity_key}` | An opportunity: its name, its statement (whose problem, and what the business gets from solving it), and its frontmatter meta. Carries `canvas_uri` when a canvas has been filled in, and `story_maps` — the maps whose key matches, as `list_story_maps` gives them. |
...
| `livt://story-map/{opportunity_key}` | The story map drawn for an opportunity: activities, steps, story cards, and releases. Story cards that have a story file link to their story resource. An opportunity has one map, filed under its key, so `{opportunity_key}` addresses it — the same key the build output uses for `story-map/{opportunity_key}.html`. Carries `opportunity_key`, and `name`: the map's own, or the key when it has none. |
