...
- **Ubiquitous language** — Eric Evans' term from Domain-Driven Design, and a thread through BDD's Formulation. livt keeps it as a glossary beside the artifacts that use it.
- **Opportunity Canvas and User Story Mapping** — Jeff Patton, [Opportunity Canvas](https://jpattonassociates.com/opportunity-canvas/) and [story mapping](https://jpattonassociates.com/story-mapping/). Not BDD, but under the same rule.
...
- **Identity.** Every rule, example, and question has an ID that is stable once filed, numbered past retired ones, and addressable as a livt URI — so a rule quoted from an Issue or a test comment cannot silently come to mean something else.
- **A lifecycle.** A rule is proposed, accepted, rejected, or retired, the way an [architecture decision record](https://adr.github.io/) is, and a retired one says where the spec went.
...
Mainstream BDD treats the feature file as the shared artifact: the Gherkin is what the Three Amigos produce together, and it is the living documentation. livt's position runs the other way — Discovery artifacts are the source of truth, and Gherkin would be generated output.
...
- **Teach BDD.** The practice is taught by the people below, and taught better.
