...
Matt Wynne の実例マッピングでは、カードは 25 分の会話のための道具で、会話が終われば役目を終えます。

livt は、実例マッピングそのものを残します。疑問点の解消やルールの変更は開発のあいだも続くので、会話に集まったスリーアミーゴスがそのまま、同じマップで完了まで追います。

<!-- figure: 上にプラクティス、下に livt。プラクティス：実例マッピング（スリーアミーゴスで 25 分）、会話が終われば役目を終える。livt：実例マッピング（スリーアミーゴス）→（疑問点の解消・ルールの変更）→ 完了、スリーアミーゴスがそのまま同じマップを完了まで追う。 -->
