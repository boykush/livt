...
<!-- figure: 上にプラクティス、下に livt。プラクティス：実例マッピング →（開発者とテスターが書き起こす）→ feature file（正、開発者・テスター）、feature file は開発とテストの手元に置かれる。livt：実例マッピング →（記録して書き換える）→ 実例マッピング形式の YAML（正、スリーアミーゴス）、そこから点線で Gherkin（要るなら生成）。 -->
