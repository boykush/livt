...
Coding agents have changed the situation. Writing the tests for an agreed rule is now work an agent can take on. Following the test pyramid, one rule's tests end up at different layers and in different languages. livt gives the agent a shape to follow and the tools to follow it, as a CLI and plugins, and works through what those differences bring with it. Teams that want their specification executed as written have Cucumber for that.
