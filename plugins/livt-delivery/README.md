...
`/inspect-automation` is gone, and nothing maintains the `automated:` flag it kept. A rule is automated when a test cites it, livt collects the citations from the implementation repository and derives the status, and there is no longer a record here for a skill to propose changes to. The flag itself outlived the station by a version: livt 0.16.0 removed the field, so a line a mapping still carries counts for nothing.
