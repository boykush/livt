...
- **The skill's, as a default** — which test cites what. [`cite-automations`](skills/cite-automations/SKILL.md) puts a citation at every level of a nested test, the rule on the block and each example on its case, and in a flat test cites a rule once all of its examples are, on each test that covers one. Counting a rule as automated because its examples are is that skill's judgment, written down as a citation; the scan reads lines and URIs and never the structure around them, and neither it nor the board infers a rule from its examples ([livt://mapping/collect-automations/rule/R-08](https://boykush.github.io/livt/demo/mapping/collect-automations.html#rule-R-08)). Your own `AGENTS.md` or skill overrides it.
