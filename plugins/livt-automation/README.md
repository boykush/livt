...
**What a claim is, where it goes by default, and nothing about how you make one true.** The line has moved twice: it used to sit at the read surface, with a livt URI in a test comment as the whole contract between the two repositories. The collect station put the claim itself on livt's side — two repositories that spell a claim differently produce a report about something other than what the tests wrote. Then where the claim sits followed it as a default: a flat test has no block for a rule's own citation, and left to every repository the answer would differ, so the board would read a rule as automated in one repository and not in the next for the same tests.
...
- **The skill's, as a default** — which test cites what. [`cite-automations`](skills/cite-automations/SKILL.md) puts a citation at every level of a nested test, the rule on the block and each example on its case, and in a flat test cites a rule once all of its examples are, without repeating it close to the example citations that already name it; each example has its own case with its citation directly above it. Counting a rule as automated because its examples are is that skill's judgment, written down as a citation; the scan reads lines and URIs and never the structure around them, and neither it nor the board infers a rule from its examples ([livt://mapping/collect-automations/rule/R-08](https://boykush.github.io/livt/demo/mapping/collect-automations.html#rule-R-08)). Your own `AGENTS.md` or skill overrides it.
- **Yours** — everything about building against it. The test framework, the comment syntax your language spells the line in, where in the file the line sits, what counts as done. The scan's blindness to structure is what spares livt any knowledge of your framework. So is taking part at all: your repository scans itself, so joining is one step added to your CI and one credential held on your side, with nothing about it declared in the livt repository.

livt still ships no test framework and no definition of done. What it does ship now is the step itself, as two actions — so the marker, the walk, the report, where it lands and what the pull request says are all livt's, and none of them is a recipe you keep a copy of. What stays yours is everything above the citation: the framework, the comment syntax, where in the file the line sits, what counts as covered — and the placement too, whenever the default skill's answer is not yours.
