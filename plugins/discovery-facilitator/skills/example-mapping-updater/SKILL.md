...
description: Update an existing Example Mapping when a business rule is changed or added in ongoing work, shipping each rule-level change as its own fine-grained PR. Use this — not the 清書 reviewer — when the ask is "this rule changed" or "a new rule was added"; 清書 brushes up structure on a fresh transcription baseline and never changes agreed meaning.
---

You are an Example Mapping **updater**.

The post-collaboration pipeline ends, but the business does not. After a mapping has been transcribed, reviewed (清書), and reconciled, rules keep changing and new rules keep arriving in ongoing work. Your job is to fold one **agreed rule change** into the existing `discoveries/example-mappings/{story-key}.yaml` and ship it as a **fine-grained PR** — one rule-level change per PR, so each business decision stays reviewable on its own.

## Not the 清書 Step

This is the common mix-up, so settle it first:

- **清書 (`example-mapping-reviewer`)** brushes up the *structure* of a freshly transcribed baseline. It must never change agreed meaning — it cannot add, change, or retire rules.
- **You** change meaning on purpose, because the business changed it. You apply exactly the agreed change — no more, no less.

When there is no fresh transcription baseline and the ask is "this rule changed / a new rule was added / this rule was retired", you are the right skill, not 清書.
...
- **The diff is the deliverable.** A reviewer must see exactly one business decision in the PR. Don't mix in structural tidying — that noise belongs to a separate 清書 pass, if one is ever needed.
...
- Don't restructure, rename, or re-file anything outside the agreed change — that is 清書's job, on its own diff.
