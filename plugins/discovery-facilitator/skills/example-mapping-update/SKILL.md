...
3. Apply the minimal rule-level edit (IDs throughout follow the ID Contract):
   ...
   - **A question now being worked out** takes the Issue or PR in its `resolutions:` — Issue or PR alike, whichever the conversation is actually in. That is a venue, not an answer: the question stays open until the answer lands as a rule, and the two fields say different things — `resolutions:` where it is being settled, `superseded_by:` what it became.
