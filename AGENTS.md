...
- `ja.go` and `ubiquitous/` name the same things and must agree. A term renamed in one is renamed in the other, or a board ends up labelled 具体例 above a glossary that calls it 実例 — the drift ubiquitous language exists to prevent, on the site whose job is to show it.
...
- A commit that changes a plugin moves its version in the same commit — `plugins/*/.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json`, which the three plugins share. The marketplace serves them by path, so merging is releasing: a `feat(plugins)!` whose version stays put leaves one number covering incompatible sets of skills, and nobody installing can tell which they got.
