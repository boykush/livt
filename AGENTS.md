...
livt turns a discovery master — story maps, example mappings, ubiquitous language — into a living document (static site, CLI, MCP server). The repo dogfoods itself: `discoveries/`, `stories/`, and `ubiquitous/` hold a real master describing livt, rendered by livt.
...
**Japanese** — the prose of the master itself:

- `stories/*.md` — `name:` and the user-story body
- `discoveries/**/*.yaml` — `name:` and `text:`
- `ubiquitous/*.md` — `name:` and the definition

The master's language is not a rule livt imposes. The skills say prose follows the language the user is speaking — see any `## Language` section under `plugins/discovery-facilitator/skills/`. *This* master happens to be Japanese, and stays that way for consistency.

Two consequences worth stating:

- Quoting the master inside English text keeps the Japanese verbatim. A commit body naming a story map writes 協働ディスカバリー, not a translation of it.
- What livt renders *of itself* — nav labels, headings, empty states — is English. The Japanese on a livt site belongs to the master being rendered, not to the tool.
...
- Master changes (`discoveries/`, `stories/`, `ubiquitous/`) ship as `docs:`. The branch prefix mirrors the type: `docs/…`, `feat/…`, `fix/…`.
